...
struct TeamFunctor {
                   ...
                   value_type sum) const 
  {
    ...
    int begin = coord_idx_offset;
    int end = begin + n_points_per_team;
    auto range = Kokkos::TeamThreadRange<>(teamMember, begin, end);

    double *teamResult =
           (double *) teamMember.team_shmem().get_shmem(value_count*sizeof(double));
    for (size_type i = 0; i < value_count; i++) teamResult[i] = 0;

    Kokkos::parallel_for(range, 
      KOKKOS_LAMBDA(int &coord_idx) {
        ...
        for(int part_idx = 0; part_idx < N_PARTS; ++part_idx) {
          ...
          {
	    Kokkos::atomic_add(&teamResult[part_idx], 1.);
          }
        }
      });
    ...
    Kokkos::single(Kokkos::PerTeam(teamMember),
      [=] ()
      {
        for (size_type j = 0; j < value_count; j++) 
          Kokkos::atomic_add(&sum[j], teamResult[j]);
      });
